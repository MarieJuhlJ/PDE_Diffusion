...
class DDPM_Scheduler(Scheduler):
    ...
    def ddim_sample(self, model_output, t, t_prev, z, sample):
        sqrt_alpha_prev_cumprod = torch.sqrt(self.alpha_cumprod[t_prev])[:, None, None, None]
        sqrt_alpha_cumprod = torch.sqrt(self.alpha_cumprod[t])[:, None, None, None]
        sqrt_1m_alpha_cumprod = torch.sqrt(1 - self.alpha_cumprod[t])[:, None, None, None]
        lhs = sqrt_alpha_prev_cumprod * ((sample - sqrt_1m_alpha_cumprod * model_output) / sqrt_alpha_cumprod)
        rhs = torch.sqrt(1 - self.alpha_cumprod[t_prev] - self.sigmas[t]**2)[:, None, None, None] * model_output
        return lhs+rhs
